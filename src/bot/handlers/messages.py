...
from tasks.celery_tasks import parse_data
...
class Messages:
    ...
    async def process_videos_amount(self, message: Message, state: FSMContext):
        ...
        parse_data.delay(telegram_user_id=message.from_user.id,
                         channel_url=link,
                         video_amount=video_amount)

        await message.answer(f"Спасибо! Видео скоро будут готовы.\n\n{result}")
